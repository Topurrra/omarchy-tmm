...
**tmm.manual** brings [The Missing Manual](https://themissingmanual.dev) into Omarchy as a native v4 Quattro plugin: a keyboard-first reader window backed by a headless search service, plus a CLI that renders the same guides in your terminal.
...
## A window, not a fullscreen overlay

tmm.manual is a normal floating Hyprland window, not a layer-shell scrim over
everything else. Summon it on an empty workspace and it opens large, like any
other app; summon it next to something already tiled and it floats or snaps
in as a side pane instead of covering the screen.

The layout is responsive to the window's own width, not the monitor's:

- **Narrower than ~900px** — a single column: type to search, arrows to
  move, `Enter` to read, same as before.
- **900px and wider** — two columns side by side: the search results or
...
- **A real reader, not a text dump** — headings, lists, block quotes, tables and rules are drawn as themed QML; code blocks are syntax-highlighted (keywords, strings, numbers, functions) in both light and dark themes, and land in bordered cards you click to copy
...
- **A bar button** — a book glyph in the Omarchy bar that toggles the window, so there is always something to click
...
| Need | Notes |
|------|-------|
| Omarchy v4 Quattro | Uses the QML `panel` + `service` plugin kinds and the `qs.Commons` / `qs.Ui` theme modules. Not compatible with v3 Walker |
| `omarchy-shell` | Hosts the window and service |
...
1. **The bar button** — a book glyph, added to the right of the bar when the
   plugin is enabled. Click it to toggle the window.
...
The window accepts an optional JSON payload:
...
```
manifest.json                  # id tmm.manual, kinds panel + service + bar-widget
Panel.qml                      # floating window: responsive search / reader / catalog
Controller.qml                 # panel state machine (mode, theme, sidebar)
BarWidget.qml                  # bar button that toggles the window
Reader.qml                     # markdown blocks drawn as themed QML, with syntax highlighting
...
components/                    # small shared QML pieces (header, footer hints, empty state, …)
...
- **Window does not appear.** Work through these in order:
  ...
  # 3. Are all the files there? Panel.qml needs its siblings
  ls ~/.config/omarchy/plugins/tmm.manual/
  # expect: manifest.json Panel.qml Controller.qml Reader.qml ResultList.qml
  #         Service.qml Model.js Markdown.js logo.png components/
...
- **Window appears unstyled**: you are on an Omarchy build without `qs.Commons` / `qs.Ui`; this plugin targets v4 Quattro.
- **Diagrams show as `Diagram · mermaid` cards instead of rendering inline**: `rsvg-convert` (librsvg) is missing or `bin/tmm-diagrams` cannot run. It ships with Omarchy, but check `command -v rsvg-convert`.
...
- **Broken image in the header**: `logo.png` did not get copied next to `Panel.qml`.
...
Keep a QML file to one job and split it when it grows a second one — that is what `Reader.qml` and `ResultList.qml` are. `Panel.qml` is deliberately the largest file: it owns the window, the responsive layout, the modes and the key map, which are hard to separate without making the flow harder to follow. (An earlier version of this note asked for ~300 lines per file; that was never true of the shell's own plugins either.)
